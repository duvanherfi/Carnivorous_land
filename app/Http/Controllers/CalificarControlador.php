...
class CalificarControlador extends Controller
{
    public function registrar(Request $request)
    {
        ...
        $request->session()->forget('datosDireccion');
        $request->session()->forget('id');
        $request->session()->forget('valorTotal');
        $sesion = $request->session()->get('id');
        for ($i=0; $i < count($sesion); $i++) { 
            $request->session()->forget('producto' . $sesion[$i]);
        }
    }
}
